...
Now you can save config and run [commands](https://www.nginx.com/resources/wiki/start/topics/tutorials/commandline/#stopping-or-restarting-nginx)
...
Trying with the `worker_rlimit_nofile` directive in `{,/usr/local}/etc/nginx/nginx.conf` fails as SELinux policy doesn't allow `setrlimit`. This is shown in `/var/log/nginx/error.log`
    ...
    20/02/24 11:16:20 [alert] 13004#0: setrlimit(RLIMIT_NOFILE, 2342) failed (13: Permission denied)
